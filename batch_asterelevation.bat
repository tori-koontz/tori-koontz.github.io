...
saga_cmd grid_tools 3 -GRIDS=W:\GEOG323_OpenSource\lab4\Attempt2\ASTER_elevation\ASTGTMV003_S03E037_dem.sgrd;W:\GEOG323_OpenSource\lab4\Attempt2\ASTER_elevation\ASTGTMV003_S04E037_dem.sgrd -NAME=%pre%mosaic -TYPE=9 -RESAMPLING=0 -OVERLAP=1 -MATCH=0 -TARGET_OUT_GRID=%od%\%pre%mosaic.sgrd
...
:: Hillshading 
saga_cmd ta_lighting 0 -ELEVATION=%od%\%pre%mosaicUTM.sgrd -SHADE=%od%\%pre%_Hillshade.sgrd -METHOD=0 -POSITION=0 -AZIMUTH=315.000000 -DECLINATION=45.000000 -DATE=2019-10-07 -TIME=12.000000 -EXAGGERATION=1.000000 -UNIT=0 -SHADOW=0 -NDIRS=8 -RADIUS=10.000000

::Sink Route Detection 
saga_cmd ta_preprocessor 1 -ELEVATION=%od%\%pre%mosaicUTM.sgrd -SINKROUTE=%od%\%pre%_sinkroute.sgrd -THRESHOLD=0 -THRSHEIGHT=100.000000

::Sink Removal 
saga_cmd ta_preprocessor 2 -DEM=%od%\%pre%mosaicUTM.sgrd -SINKROUTE=%od%\%pre%_sinkroute.sgrd -DEM_PREPROC=%od%\%pre%_sinkremoved.sgrd -METHOD=1 -THRESHOLD=0 -THRSHEIGHT=100.000000

::Flow Accumulation 
saga_cmd ta_hydrology 0 -ELEVATION=%od%\%pre%_sinkremoved.sgrd -SINKROUTE=%od%\%pre%_sinkroute.sgrd -WEIGHTS=NULL -FLOW=%od%\%pre%_FlowAccu.sgrd -VAL_INPUT=NULL -ACCU_MATERIAL=NULL -STEP=1 -FLOW_UNIT=0 -FLOW_LENGTH=NULL -LINEAR_VAL=NULL -LINEAR_DIR=NULL -METHOD=4 -LINEAR_DO=1 -LINEAR_MIN=1000 -CONVERGENCE=1.100000
